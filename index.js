...
const getMySQLProcedures = () => {
    ...
    if(!found){    
        try {
            ...
            console.log("proceduresMethods folder not found returning empty procedures");
            return {};
        }
    }
    const methods = {};
...
const connect = async (ssh = sshDefault, MySQL = MySQLDefault) => {
    ...
    return new Promise((resolve, reject) => {
        mysqlssh
            ...
            .then(client => {
                db = client;
                spinner.succeed(`Connected successfully to ${MySQL.database} MySQL database`);
                resolve();
            })
            .catch(err => {
        ...
        if (!error) {
            switch (typeof (arg)) {
                ...
                case 'object':
                    ...
                    break;
                default:
                    error = new Error('Error: You tried to send either a function, symbol, bigInt\nParameter: ' + args);
                    break;
            }
            comma = ',';
        }
...
const queryProcedure = async (...args) => {
    return new Promise((resolve, reject) => {
        ...
        if (!parsedArgs.error) {
            db.promise().query(`CALL ${args[0]}(${parsedArgs.str})`).then((res) => {
                resolve(res[0][0]);
            }).catch((err) => {
                reject(err);
            });
...
const emptyTestDatabase = async () => {
    return new Promise((resolve, reject) => {
        if (!db.config.database.includes('test')) return reject('CRITICAL WARNING, YOU TRIED TO TRUNCATE A NON-TESTING DATABASE');
